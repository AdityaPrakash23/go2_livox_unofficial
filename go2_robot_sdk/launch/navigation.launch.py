...
def generate_launch_description():
    ...
    launch_args = [
        ...
        DeclareLaunchArgument('fast_lio_force_2d', default_value='true', description='Flatten FAST-LIO odometry to planar x/y/yaw for Nav2'),
        DeclareLaunchArgument('fast_lio_position_deadband', default_value='0.0', description='Suppress smaller FAST-LIO x/y odom jitter in meters'),
        DeclareLaunchArgument('fast_lio_yaw_deadband', default_value='0.0', description='Suppress smaller FAST-LIO yaw jitter in radians'),
        DeclareLaunchArgument('fast_lio_smoothing_alpha', default_value='1.0', description='FAST-LIO odom smoothing factor, 1.0 disables smoothing'),
        DeclareLaunchArgument('fast_lio_max_position_jump', default_value='0.0', description='Ignore single FAST-LIO position jumps larger than this in meters; <=0 disables'),
        DeclareLaunchArgument('fast_lio_max_yaw_jump', default_value='0.0', description='Ignore single FAST-LIO yaw jumps larger than this in radians; <=0 disables'),
        ...
        DeclareLaunchArgument('livox_x', default_value='0.5', description='Livox x offset from base_link in meters'),
        DeclareLaunchArgument('livox_y', default_value='0.0', description='Livox y offset from base_link in meters'),
        DeclareLaunchArgument('livox_z', default_value='0.1', description='Livox z offset from base_link in meters'),
    ...
    core_nodes = [
        ...
        Node(
            ...
            parameters=[{
                ...
                'publish_tf': True,
                'restamp_with_current_time': LaunchConfiguration('restamp_sensor_data'),
                'force_2d': LaunchConfiguration('fast_lio_force_2d'),
                'position_deadband': LaunchConfiguration('fast_lio_position_deadband'),
                'yaw_deadband': LaunchConfiguration('fast_lio_yaw_deadband'),
                'smoothing_alpha': LaunchConfiguration('fast_lio_smoothing_alpha'),
                'max_position_jump': LaunchConfiguration('fast_lio_max_position_jump'),
                'max_yaw_jump': LaunchConfiguration('fast_lio_max_yaw_jump'),
            }],
        ),
